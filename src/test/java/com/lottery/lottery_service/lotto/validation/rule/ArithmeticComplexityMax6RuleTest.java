 ...
 *
 * <ul>
 *   <li>정렬된 6개 숫자의 모든 (aj - ai) 차이값을 구한다.
 *   <li>서로 다른 차이값 개수(AC)를 계산한다.
 *   <li>AC가 6 이하면 false, 7 이상이면 true를 반환한다.
 * </ul>
 *
 * <p>현재 브랜치 기준으로 invalid input(null, 개수 불일치 등)은 개별 룰 테스트에서 반복 검증하지 않고, 도메인 규칙 의미 자체만 검증한다.
 */
class ArithmeticComplexityMax6RuleTest {
   ...
   *
   * <p>예: 1, 2, 4, 8, 16, 32 -> pairwise diff가 매우 다양해서 AC > 6
   *
   * <p>입력을 일부러 정렬되지 않은 순서로 넣어, 룰 내부의 sort가 실제로 동작해도 결과가 올바른지 함께 본다.
   */
  @Test
   ...
   * 차이값 종류가 적으면 실패해야 한다.
   *
   * <p>예: 1, 2, 3, 4, 5, 6 -> 가능한 차이값 종류는 {1,2,3,4,5} 이므로 AC=5 -> AC<=6 이므로 실패
   */
  @Test
   ...
   *
   * <p>이 테스트는 룰이 "AC가 작다"는 이유로 등차수열 같은 단순 패턴을 잘 차단하고 있음을 더 명확히 보여준다.
   */
  @Test
