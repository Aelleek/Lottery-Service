 ...
 * <p>이 테스트 클래스의 역할:
 * <ul>
 *   <li>DB 전체 로드 결과를 캐시에 올바르게 반영하는지 검증한다.</li>
 *   <li>증분 추가(add)가 기존 캐시를 유지한 채 새 값을 포함시키는지 검증한다.</li>
 *   <li>contains(...)가 현재 캐시 상태를 정확히 조회하는지 검증한다.</li>
 * </ul>
 *
 * <p>중요:
 * <ul>
 *   <li>이 테스트는 실제 DB를 사용하지 않는다.</li>
 *   <li>Repository는 mock으로 두고, cache의 메모리 상태 전이만 검증한다.</li>
 *   <li>동시성 자체를 직접 검증하지는 않지만, 원자적 교체 설계가 의도대로 동작하는지 확인한다.</li>
 * </ul>
 */
...
class LottoWinnerDataCacheTest {
  ...
  @Mock
  private LottoWinnerDataRepository repo;

  @InjectMocks
  private LottoWinnerDataCache cache;

  /**
   * reloadAll()은 repository.findAll() 결과의 winnerData만 뽑아서
   * 현재 캐시를 전량 교체해야 한다.
   *
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>repo.findAll()이 호출된다.</li>
   *   <li>DB에 있는 canonical 문자열들이 캐시에 반영된다.</li>
   *   <li>reloadAll() 호출 전 캐시에 있던 임시 값은 유지되지 않는다.</li>
   * </ul>
   */
  @Test
  ...
  void reloadAll_repositoryHasData_replacesCacheWithAllWinnerData() {
    // given
    LottoWinnerData round1111 = LottoWinnerData.builder()
        .round(1111)
    ...
    LottoWinnerData round1112 = LottoWinnerData.builder()
        .round(1112)
   ...
   *   <li>기존 값은 유지된다.</li>
   *   <li>새 값이 추가된다.</li>
   *   <li>DB를 다시 조회하지 않고 메모리 캐시만 갱신한다.</li>
   * </ul>
   */
  @Test
   ...
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>중복 add가 발생해도 contains 결과는 안정적으로 true다.</li>
   *   <li>Set 의미론을 깨뜨리지 않는다.</li>
   * </ul>
   *
   * <p>참고:
   * 내부 Set의 size를 외부에서 직접 볼 수 없으므로,
   * 이 테스트는 "중복 추가가 이상 동작을 만들지 않는다"는 관점으로 검증한다.
   */
  @Test
   ...
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>초기 캐시는 빈 Set이다.</li>
   *   <li>존재하지 않는 값 조회 시 false를 반환한다.</li>
   * </ul>
   */
  @Test
   ...
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>애플리케이션 시작 시 수행되는 초기화 동작이 실질적으로 전체 리로드와 동일하다.</li>
   *   <li>init() 호출만으로 DB 데이터가 캐시에 올라온다.</li>
   * </ul>
   */
  @Test
