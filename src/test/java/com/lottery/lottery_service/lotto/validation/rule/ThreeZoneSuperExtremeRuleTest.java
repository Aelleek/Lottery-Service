 ...
 *
 * <ul>
 *   <li>번호 6개를 3구간(1~15 / 16~30 / 31~45)으로 나눈다.
 *   <li>한 구간에 6개가 전부 몰린 6-0-0 패턴이면 false를 반환한다.
 *   <li>그 외 분포는 true를 반환한다.
 * </ul>
 *
 * <p>현재 브랜치 기준으로 invalid input(null, 개수 불일치, 범위 밖 숫자 등)은 개별 룰 테스트에서 반복 검증하지 않고, 도메인 규칙 의미 자체만 검증한다.
 */
class ThreeZoneSuperExtremeRuleTest {
   ...
   *
   * <p>예: 1, 7, 16, 24, 31, 45 -> z1=2, z2=2, z3=2 -> 6-0-0 패턴이 아니므로 통과
   */
  @Test
   ...
   *
   * <p>예: 1, 2, 3, 4, 5, 6 -> z1=6, z2=0, z3=0 -> super extreme 패턴이므로 실패
   */
  @Test
   ...
   *
   * <p>이 테스트는 룰이 "극단적 분포 전체"를 막는 게 아니라, 오직 super extreme(6-0-0)만 차단한다는 현재 정책을 고정한다.
   */
  @Test
