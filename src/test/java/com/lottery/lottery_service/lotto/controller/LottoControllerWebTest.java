 ...
 *
 * <ol>
 *   <li>요청 URL과 HTTP Method가 현재 컨트롤러 계약과 일치하는지</li>
 *   <li>회원 전용 API가 인증 principal(memberId 포함)을 요구하는지</li>
 *   <li>비회원 API가 로그인 없이도 접근 가능한지</li>
 *   <li>컨트롤러가 요청을 해석한 뒤 올바른 서비스 메서드로 위임하는지</li>
 * </ol>
 *
 * <p>중요:
 * <ul>
 *   <li>이 클래스는 서비스 내부 로직(랜덤 번호 생성, 저장 정책, validation rule)을 검증하지 않는다.</li>
 *   <li>그런 로직은 별도의 LottoServiceTest, LottoValidationPipelineTest에서 검증해야 한다.</li>
 *   <li>여기서는 오직 "웹 계층 계약"만 본다.</li>
 * </ul>
 *
 */
...
class LottoControllerWebTest {
   ...
   *
   * <p>이 mock을 통해 "컨트롤러가 어떤 인자를 서비스로 넘겼는지"와
   * "서비스 결과를 응답으로 어떻게 직렬화하는지"만 검증한다.
   */
  @MockBean
   ...
   *
   * <p>실제 테스트 메서드에서 직접 사용하지 않더라도,
   * 컨트롤러 빈 생성에 필요하므로 mock bean으로 등록해야 한다.
   */
  @MockBean
   ...
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>현재 회원 추천 URL이 /api/lotto/recommendations 인지</li>
   *   <li>인증 principal 안의 memberId가 서비스로 전달되는지</li>
   *   <li>서비스가 반환한 LottoSet 5개가 JSON 배열로 응답되는지</li>
   *   <li>각 세트가 numbers 필드를 통해 직렬화되는지</li>
   * </ul>
   *
   * <p>이 테스트가 보장하지 않는 것:
   * <ul>
   *   <li>번호 생성 알고리즘의 정확성</li>
   *   <li>validation rule의 PASS/FAIL 동작</li>
   *   <li>DB 저장 성공 여부</li>
   * </ul>
   */
  @Test
  ...
  void recommendForMember_success() throws Exception {
    ...
    mockMvc.perform(post("/api/lotto/recommendations")
   ...
   *
   * <p>참고:
   * Spring Security 설정에 따라 비인증 응답은 302 redirect 또는 401이 될 수 있다.
   * 현재 설정에서는 일반적으로 3xx redirect가 나올 가능성이 높다.
   * 만약 실제 실행 결과가 401이라면 이 기대값만 맞춰 바꾸면 된다.
   */
  @Test
   ...
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>현재 조회 URL이 /api/lotto/members/me/recommendations 인지</li>
   *   <li>principal의 memberId가 서비스로 전달되는지</li>
   *   <li>LottoRecordResponse가 JSON 배열 구조로 직렬화되는지</li>
   * </ul>
   *
   * <p>즉 이 테스트는 과거 path variable 기반 계약이 아니라,
   * "로그인 사용자 자신의 추천 내역을 조회한다"는 현재 계약을 고정한다.
   */
  @Test
  @DisplayName("회원 추천 내역 조회 성공: 현재 로그인 사용자의 추천 내역을 반환한다")
  void getMyRecommendations_success() throws Exception {
    List<LottoRecordResponse> histories = List.of(
    ...
    mockMvc.perform(get("/api/lotto/members/me/recommendations")
            .with(oauth2Login().attributes(attributes -> attributes.put("memberId", 1L))))
        .andExpect(status().isOk())
   ...
   *
   * <p>회원의 저장된 추천 기록은 개인정보/개인 이력 성격이 있으므로,
   * 로그인 없이 접근되면 안 된다는 점을 보장한다.
   */
  @Test
   ...
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>현재 구매 저장 URL이 /api/lotto/purchases 인지</li>
   *   <li>JSON 요청 본문이 PurchaseLottoRequest로 바인딩되는지</li>
   *   <li>principal의 memberId와 request body가 서비스로 위임되는지</li>
   *   <li>성공 시 200 OK를 반환하는지</li>
   * </ul>
   *
   * <p>이 테스트는 "실제로 DB에 저장됐는지"를 보지 않는다.
   * 그건 서비스 테스트에서 검증한다.
   */
  @Test
  ...
  void purchase_success() throws Exception {
    ...
    mockMvc.perform(post("/api/lotto/purchases")
            .with(oauth2Login().attributes(attributes -> attributes.put("memberId", 1L)))
   ...
   *
   * <p>구매 번호 저장은 개인 계정 데이터 변경이므로,
   * 로그인 없이 호출되면 안 된다는 점을 보장한다.
   */
  @Test
   ...
   * <ul>
   *   <li>round: 비어 있으면 안 됨</li>
   *   <li>numbersList: null 불가, 최소 1개 이상</li>
   *   <li>numbersList 각 문자열: "1~45 숫자 6개를 쉼표로 구분한 문자열"이어야 함</li>
   * </ul>
   *
   ...
   * 비회원 추천 성공 케이스.
   *
   * <p>이 테스트는 "guest 추천 API는 로그인 없이 접근 가능해야 한다"는 정책을 고정한다.
   * 즉 이 테스트가 통과하려면 SecurityConfig에서
   * /api/lotto/guests/recommendations 경로가 permitAll로 열려 있어야 한다.
   *
   * <p>이 테스트가 보장하는 것:
   * <ul>
   *   <li>guest 추천 URL이 /api/lotto/guests/recommendations 인지</li>
   *   <li>로그인 principal 없이도 200 OK가 가능한지</li>
   *   <li>서비스 반환 결과가 로또 번호 5세트로 직렬화되는지</li>
   * </ul>
   */
  @Test
