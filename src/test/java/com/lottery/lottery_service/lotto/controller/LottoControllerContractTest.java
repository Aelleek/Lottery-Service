...
/**
 * LottoControllerContractTest.java
 * 명세 검증용 테스트 클래스 (익명 클래스로 LottoService 대체)
 */
@SpringBootTest
...
class LottoControllerContractTest {
    ...
    void recommendLottoForMember_shouldReturnValidJson() throws Exception {
        mockMvc.perform(post("/api/lotto/members/1/recommendations")
                        .param("source", "BASIC"))
    ...
    void getLottoRecommendations_shouldReturnLottoRecordList() throws Exception {
        mockMvc.perform(get("/api/lotto/members/1/recommendations"))
                .andExpect(status().isOk())
    ...
    void addPurchasedLotto_shouldReturnOk() throws Exception {
        ...
        mockMvc.perform(post("/api/lotto/1/purchases")
