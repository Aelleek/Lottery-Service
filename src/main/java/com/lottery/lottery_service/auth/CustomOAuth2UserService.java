 ...
 * (provider, providerUserId) 기준으로 Member/Link를 업서트한다.
 *
 * - 엔티티 직접 new/setter 대신 도메인 팩토리 & 애그리게잇 메소드 사용
 *   (Member.newMember / Member.addOAuthLink / MemberOAuthAccount.newLink)
 */
@Service
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    /**
     * 1) registrationId로 추출기 선택 → OAuthUserInfo 추출
     * 2) (provider, providerUserId)로 기존 링크 조회 → 없으면 생성/연결
     * 3) lastLoginAt 갱신 및 Security Principal 반환
     */
    @Override
    ...
    private Member createOrLinkMember(OAuthProvider provider, OAuthUserInfo info) {
        ...
        Member target = null;
        if (info.email() != null) {
            target = memberRepository.findByEmail(info.email())
        ...
        MemberOAuthAccount link = target.addOAuthLink(
                provider,
                info.providerUserId(),
                info.email(),
                info.nickname(),
