...
/**
 * OAuth2User → OAuthUserInfo 변환기 인터페이스.
 * 각 제공자별 구현체는 supportsRegistrationId()에 자신이 처리할 registrationId를 반환한다.
 */
public interface OAuth2UserInfoExtractor {

    /**
     * 이 추출기가 처리하는 registrationId를 반환.
    ...
    /**
     * 주어진 registrationId를 이 추출기가 처리 가능한지 여부.
     * 기본 구현은 문자열 일치(대소문자 무시)로 판단한다.
     */
    default boolean supports(String registrationId) {
        return registrationId != null
                && registrationId.equalsIgnoreCase(supportsRegistrationId());
    }
}