...
import com.lottery.lottery_service.lotto.external.DhlotteryClient;
 ...
 * <p>동작:
 * <ul>
 *   <li>앱 시작 시: DB의 최신 round 확인 → 다음 round부터 API로 조회/저장(미발표 시 중단)</li>
 *   <li>추후 스케줄링에서도 {@link #syncMissingRoundsOnce()} 재사용</li>
 * </ul>
 */
@Service
...
public class LottoWinnerSyncService {
     ...
     * 앱 기동 완료 후 한 번 실행.
     * 추후에 이관 예정
     * DB 최신 회차 이후의 누락분을 API에서 조회해 저장한다.
     */
    ...
    /**
     * DB 최신 round의 다음 회차부터, API가 '미발표/없음'을 반환할 때까지
     * 순차적으로 조회해 저장한다.
     *
     * @return 저장된 건수
     */
