 ...
 *
 * 책임(Responsibility)
 * - 주입된 룰들을 @Order 순서대로 실행한다.
 * - disabled 룰은 건너뛴다.
 * - 첫 FAIL에서 즉시 중단(Short-circuit)하고, 실패한 룰의 ID/사유코드와
 *   지금까지 실행(시도)한 룰 ID 목록을 LottoValidationResult에 담아 반환한다.
 * - 모든 활성 룰 통과 시 PASS 결과를 반환한다.
 *
 * 비책임(Non-Responsibility)
 * - 개별 도메인 판정 로직(과거 1등과 동일 여부 등)은 룰 구현체가 담당한다.
 */
@Component
public class LottoValidationPipeline {
    ...
    /**
     * 실행할 룰 목록.
     * - 생성자에서 불변 정렬 리스트로 고정하여, 런타임 중 외부 변경을 방지한다.
     */
