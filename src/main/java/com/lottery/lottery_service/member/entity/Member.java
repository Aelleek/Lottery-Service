...
/**
 * 서비스 고유 사용자 엔티티. - 소셜 제공자와 무관한 '우리 서비스의 프로필'을 표현한다. - email은 카카오처럼 미제공/동의거부일 수 있으므로 nullable 허용. -
...
@Table(
    name = "member",
    indexes = {@Index(name = "idx_member_last_login_at", columnList = "lastLoginAt")})
public class Member {
  ...
  /** Member를 생성할 때 필수값을 한 번에 설정하는 정적 팩토리. email은 null 가능(카카오 동의 거부 대비), emailVerified는 기본 false. */
  ...
  /** (provider, providerUserId)로 소셜 링크를 생성하고, 양방향 연관관계를 한 번에 맞춰준다. */
  public MemberOAuthAccount addOAuthLink(
