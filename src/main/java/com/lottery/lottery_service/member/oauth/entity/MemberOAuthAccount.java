...
/**
 * 소셜 제공자 계정과 Member를 연결하는 엔티티. (provider, provider_user_id) 조합이 전역적으로 유일하다.
 *
 * <p>지금 단계에선 provider를 String으로 둔다. (STEP 4에서 enum으로 교체)
 */
@Getter
@Setter
...
public class MemberOAuthAccount {
  ...
  /**
   * 링크 엔티티를 생성하는 정적 팩토리. - 정체성 필드(provider, providerUserId)는 생성 시에만 세팅되고 이후 변경 불가. - member 역참조까지
   * 세팅한다.
   */
  public static MemberOAuthAccount newLink(
      Member owner,
